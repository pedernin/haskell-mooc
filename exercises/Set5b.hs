...
search val (Node a l r) 
  | val == a = Just []
  | otherwise = helper (search val l) (search val r)
    where 
    helper :: Maybe [Step] -> Maybe [Step] -> Maybe [Step]
    helper Nothing (Just rp) = Just (StepR:rp)
    helper (Just lp) Nothing = Just (StepL:lp)
