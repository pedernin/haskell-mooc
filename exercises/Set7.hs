...
import Test.Hspec.Discover (Semigroup)
...
member val (Set set) = val `elem` set

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add val (Set xs) = Set (go val xs)
  where
    go x [] = [x]
    go x (y:ys)
      | x == y = (y:ys)
...
data State = Start | EggsAdded | FlourAdded | SugarAdded | Mixed  | Error | Finished | FlourAndSugarAdded
  deriving (Eq,Show)
...
reverseNonEmpty (x :| []) = x :| []
reverseNonEmpty (x :| xs) = head reversedList :| tail reversedList
  where reversedList = reverse (x:xs)
...
passwordAllowed password (ContainsSome x) = any (`elem` password) x
passwordAllowed password (DoesNotContain x) = all (`notElem` password) x
