...
rangeOf xs = abs (minimum xs - maximum xs)
...
winner scores player1 player2
    ...
    | otherwise = player2
    where score player = findWithDefault 0 player scores
...
freqs = foldr (Map.alter something) Map.empty
    where something = Just . maybe 1 (+1)
