...
leftpad s n
    | length s >= n = s
    | otherwise = concat (replicate (n - length s) " ") ++ s
...
helper 0 s = s ++ "Liftoff!"
helper n s = helper (n-1) (s ++ show n ++ "... ")
...
helper1 :: Integer -> Integer -> Integer
helper1 n k
    | n `mod` k == 0 = k
    | otherwise = helper1 n (k+1)
smallestDivisor n = helper1 n 2
