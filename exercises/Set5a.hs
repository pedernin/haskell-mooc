...
totalPrice (MkShoppingEntry _name itemPrice count) = itemPrice * fromIntegral count
...
buyOneMore (MkShoppingEntry name itemPrice count) = MkShoppingEntry name itemPrice (count+1)
...
getX :: Position -> Int
getX (MkPosition x _y) = x

-- getY returns the y of a position
getY :: Position -> Int
getY (MkPosition _x y) = y
...
data UpDown = IncCounter Int | DecCounter Int
...
get :: UpDown -> Int
get (IncCounter val) = val
get (DecCounter val) = val

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (IncCounter val) = IncCounter (val+1)
tick (DecCounter val) = DecCounter (val-1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (IncCounter val) = DecCounter val
toggle (DecCounter val) = IncCounter val
...
fromList [] = Empty
fromList ((key,val):kvs) = Pair key val (fromList kvs)
...
toNat z
  | z == 0 = Just Zero
  | z < 0 = Nothing
  | otherwise = Just (helper z)
    where
      helper 0 = Zero
      helper z = PlusOne (helper( z-1))
...
prettyPrint (I b) = prettyPrint b ++ "I"
